...
const PropertyCard = ({ listing, viewMode }) => {
  ...
  return (
    ...
    //             <div className={`flex flex-col ${viewMode == 'grid' ?  'mx-auto' : 'mx-0 h-44'} bg-[#05A650] text-white p-2 shadow-lg z-14 `}>
    ...
    <div
      // key={key}
      className={`relative rounded-lg overflow-visible ${viewMode == "grid"
          ? "flex flex-col"
    ...
    >
      <Link href={`/property/${listing.slug}`} className="w-full h-full">
        ...
        {/* Card Body */}
        <div
          className={`flex flex-col justify-between bg-[#05A650] text-white p-3 
